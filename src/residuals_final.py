...
class sva:
    ...
    def __init__(self, filename, psub, nperm, alpha):
        self.data = pd.read_csv(filename,sep='\t')
        self.sub = float(psub)
        self.perm = int(nprocs)
    ...
    def subset_svd(arr,plist,lam,resarr):
        ...
        trends = []
        for j, entry in enumerate(self.ps):
            sub = []
    ...
    def normalize(self,outname):
        fin_res = np.dot(linalg.lstsq(np.asarray(self.ts).T,self.data.values.T)[0].T,np.asarray(self.ts))
...
def main(argv):
    ...
    outputfile = ''
    try:
        opts, args = getopt.getopt(argv,"h:i:o:m:p:",["ifile=","ofile=","sub=","perm=","alpha="])
    except getopt.GetoptError:
        print 'knn_impute.py -i <inputfile> -o <outputfile> -s <subset%> -p <#permutations> -a <alphalevel>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print 'knn_impute.py -i <inputfile> -o <outputfile> -s <subset%> -p <#permutations> -a <alphalevel>'
