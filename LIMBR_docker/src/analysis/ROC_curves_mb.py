...
def main(argv):
    ...
    outdata = None
    try:
        outdata = pd.read_csv('output/simdata/simdata_mb.csv').values.tolist()
    except:
        pass
    ...
    pd.DataFrame(outdata, columns=['Iteration','Base_FPR','Base_TPR','Circ_FPR','Circ_TPR','TS_FPR','TS_TPR','Block_FPR','Block_TPR','Noise_FPR','Noise_TPR','Base_auc','Circ_auc','TS_auc','Block_auc','Noise_auc']).to_csv('output/simdata/simdata_mb.csv',index=False)
